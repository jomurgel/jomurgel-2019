...
add_filter( 'body_class', 'JoTheme\Functions\body_classes' );
...
add_action( 'wp_enqueue_scripts', 'JoTheme\Functions\remove_front_end_jquery' );
...
function add_og_tags() {
	...
	$default_base_description = ( get_bloginfo( 'description' ) ) ? get_bloginfo( 'description' ) : esc_html__( 'Visit our website to learn more.', 'jo-theme' );
	...
	if ( is_category() || is_tag() || is_tax() ) {
		...
		$specify        = ( is_category() ) ? esc_html__( 'categorized in', 'jo-theme' ) : esc_html__( 'tagged with', 'jo-theme' );
		$queried_object = get_queried_object();
		$card_url       = get_term_link( $queried_object );
		$card_type      = 'website';

		// Translators: get the term name.
		$card_long_description = $card_description = sprintf( esc_html__( 'Posts %1$s %2$s.', 'jo-theme' ), $specify, $term_name ); // phpcs:ignore Squiz.PHP.DisallowMultipleAssignments.Found
	}
	...
	if ( is_search() ) {
		...
		$card_long_description = $card_description = sprintf( esc_html__( 'Search results for %s.', 'jo-theme' ), $search_term ); // phpcs:ignore Squiz.PHP.DisallowMultipleAssignments.Found
	}
...
add_action( 'wp_head', 'JoTheme\Functions\add_og_tags' );
...
add_filter( 'get_the_archive_title', 'JoTheme\Functions\remove_archive_title_prefix' );
...
function category_transient_flusher() {
	...
	// Like, beat it. Dig?
	delete_transient( 'jo_theme_categories' );
}
add_action( 'delete_category', 'JoTheme\Functions\category_transient_flusher' );
add_action( 'save_post', 'JoTheme\Functions\category_transient_flusher' );
...
add_filter( 'the_content', 'JoTheme\Functions\get_new_content', 20 );
