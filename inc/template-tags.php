...
use function JoTheme\SocialIcons\get_social_icon_array;

require_once JO_THEME_PATH . '/inc/social-icons.php';
...
function display_posted_on( $relative = true ) {
	...
	$posted_on = sprintf(
		...
		esc_html_x( 'Posted on %s', 'post date', 'jo-theme' ),
...
function display_relative_time( $args = array() ) {
	...
	$time_string = $time_space ? implode( ', ', $time_space ) . esc_html__( ' ago', 'jo-theme' ) : esc_html__( 'just now', 'jo-theme' );
...
function display_site_branding() {
	...
	$logo = get_social_icon_array()['logo'];

	// Define markup tag.
	$title_heading = is_front_page() && is_home() ? 'h1' : 'p';

	// Set branding output value.
	$site_heading = $logo ? $logo : $blog_info;

	// Construct markup.
	echo sprintf(
		'<%1$s class="h1 site-title"><a href="%2$s" rel="home">%3$s</a></%1$s>', esc_attr( $title_heading ), esc_url( home_url( '/' ) ), get_new_content( $site_heading )
	); // WPCS: XSS Ok.
}
 ...
 * @return void
 */
function display_footer_mark() {

	$logo = get_social_icon_array()['logo_slim'];

	// Bail if we don't have a logo.
	if ( ! $logo ) {
		return;
	}
	?>
	...
	<div class="footer-logo">
		<?php echo get_new_content( $logo ); // WPCS: XSS ok. ?>
	</div><!-- .footer-logo -->
...
function display_page_title() {
	...
	if ( is_singular() ) {
		...
		$heading = /* translators: the term(s) searched */ sprintf( esc_html__( 'Search Results for: %s', 'jo-theme' ), '<span class="query-term">' . get_search_query() . '</span>' );
	} elseif ( is_404() ) {
		$heading = esc_html__( '¯\_(ツ)_/¯', 'jo-theme' );
...
function display_search_toggle() {
	...
	<div id="search-form-container" class="search-form-container" role="search" aria-label="<?php esc_attr_e( 'Sitewide Search', 'jo-theme' ); ?>" aria-hidden="true">
		<?php get_search_form(); ?>
...
function display_read_more() {
	...
	// Construct markup.
	echo sprintf(
		'<p class="read-more"><a href="%1$s" rel="bookmark" aria-label="%2$s">%3$s</a></p>', esc_url( get_the_permalink() ), esc_html__( 'Continue Reading ', 'jo-theme' ) . esc_html( get_the_title() ), esc_html__( '. . .', 'jo-theme' )
	);
}
...
function display_numeric_pagination( $args = array() ) {
	...
	$defaults = array(
		'mid_size'  => 4,
		'prev_text' => __( 'Previous', 'jo-theme' ),
		'next_text' => __( 'Next', 'jo-theme' ),
		'type'      => 'list',
	);
...
function display_post_footer() {
	...
	$output .= '<li><span class="share-to">' . esc_html__( 'Share on: ', 'jo-theme' ) . '</span></li>';
...
function display_footer_copyright() {
	...
	echo sprintf( '&copy; %s <span class="copyright-language">%s</span>', esc_html( date( 'Y' ) ), get_new_content( $copy_language ) ); // WPCS: XSS Ok.
}
...
function display_category_list() {
	...
	if ( ! $categories || is_search() || is_page() || is_404() || is_home() || is_front_page() ) {
		return;
	}
	?>

	<aside class="category-list">
		<h4><?php esc_html_e( 'Additional Content', 'jo-theme' ); ?></h4>
		<ul>
		<?php
